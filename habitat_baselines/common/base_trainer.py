...
class BaseTrainer:
    ...
    def load_model(self) -> Dict:
        ...
        assert os.path.isfile(self.config.EVAL_CKPT_PATH_DIR), (
            f"Not found " f"path {self.config.EVAL_CKPT_PATH_DIR}"
        )

        if os.path.isfile(self.config.EVAL_CKPT_PATH_DIR):
            # evaluate singe checkpoint
            self.ckpt_dict = self.load_checkpoint(self.config.EVAL_CKPT_PATH_DIR,
                                                  map_location="cpu")

    def eval(self, img=None, goal=None):
        ...
        if os.path.isfile(self.config.EVAL_CKPT_PATH_DIR):
            ...
            action = self._eval_checkpoint(self.config.EVAL_CKPT_PATH_DIR,
