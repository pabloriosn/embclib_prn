...
class PPOTrainer(BaseRLTrainer):
    ...
    def _eval_checkpoint(self, checkpoint_path: str, checkpoint_index: int = 0, image=None, goal = None):
        ...
        if self.aux:
            ...
            self.test_recurrent_hidden_states = torch.zeros(self.config.NUM_ENVIRONMENTS, self.actor_critic.net.num_recurrent_layers, ppo_cfg.hidden_size, device=self.device, )

            self.prev_actions = torch.zeros(self.config.NUM_ENVIRONMENTS, 1, device=self.device, dtype=torch.long, )
        ...
        self.observations[0]['rgb'] = image
        self.observations[0]['objectgoal'] = np.array([goal])
